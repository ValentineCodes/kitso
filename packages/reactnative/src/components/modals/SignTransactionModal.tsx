...
export default function SignTransactionModal({ modal: { closeModal, params } }: Props) {
    ...
    return (
        <VStack bgColor="white" borderRadius="30" p="5" space={4} w={WINDOW_WIDTH * 0.9}>
            <VStack space="2">
                ...
                <HStack alignItems="center" justifyContent="space-between" bgColor="#F5F5F5" borderRadius="10" p="2">
                    <HStack alignItems="center" space="2">
                        ...
                        <VStack w="75%">
                            <Text fontSize={FONT_SIZE['xl']} fontWeight="medium">{profile?.name}</Text>
                            <Text fontSize={FONT_SIZE['md']}>Balance: {balance && `${balance} ${network.token}`}</Text>
                        </VStack>
                    </HStack>
                </HStack>
            </VStack>
            ...
            <Text fontSize={2 * FONT_SIZE["xl"]} bold textAlign="center">{ethers.utils.formatEther(params.value)} {network.token}</Text>
            ...
            <VStack borderWidth="1" borderColor="muted.300" borderRadius="10">
                <HStack p="3" alignItems="flex-start" justifyContent="space-between">
                    ...
                    <VStack w="50%">
                        <Text fontSize={FONT_SIZE['lg']} fontWeight="medium" textAlign="right">{estimatedGasCost.min && parseFloat(ethers.utils.formatEther(estimatedGasCost.min), 8)} {network.token}</Text>
                        <Text fontSize={FONT_SIZE['md']} fontWeight={"semibold"} textAlign="right" color={"muted.500"}>Max fee:</Text>
                        <Text fontSize={FONT_SIZE['md']} textAlign="right">{estimatedGasCost.max && parseFloat(ethers.utils.formatEther(estimatedGasCost.max), 8)} {network.token}</Text>
                    </VStack>
                </HStack>
                ...
                <HStack p="3" alignItems="flex-start" justifyContent="space-between">
                    ...
                    <VStack w="50%">
                        <Text fontSize={FONT_SIZE['lg']} fontWeight="medium" textAlign="right">{calcTotal().min} {network.token}</Text>
                        <Text fontSize={FONT_SIZE['md']} fontWeight={"semibold"} textAlign="right" color={"muted.500"}>Max amount:</Text>
                        <Text fontSize={FONT_SIZE['md']} textAlign="right">{calcTotal().max} {network.token}</Text>
                    </VStack>
                </HStack>
            </VStack>
