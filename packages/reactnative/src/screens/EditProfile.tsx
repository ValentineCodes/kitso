...
export default function EditProfile({ }: Props) {
    ...
    const [links, setLinks] = useState<({ id: string } & LinkType)[]>(profile?.links.map((link, index) => ({...link, id: index.toString()})) || [])
    ...
    const selectCoverImage = () => {
        if(coverImage){
            ...
            return { uri: profile.backgroundImage[0].url.replace("ipfs://", network.ipfsGateway) }
        } else {
    ...
    const selectProfileImage = () => {
        if(profileImage){
            ...
            return { uri: profile.profileImage[0].url.replace("ipfs://", network.ipfsGateway) }
        } else {
