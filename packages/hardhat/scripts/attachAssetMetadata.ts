...
async function uploadToPinata(imagePath: string): Promise<{ ipfsHash: string; bufferHash: string } | null> {
  try {
...
async function uploadJSONToPinata(jsonData: any): Promise<string | null> {
  try {
    ...
    const { data } = await axios.post("https://api.pinata.cloud/pinning/pinJSONToIPFS", body, {
      headers: {
...
export async function attachAssetMetadata(assetAddress: string, imagePath: string) {
